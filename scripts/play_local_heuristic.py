#!/usr/bin/env python3
"""
Play pokejax PPO model vs poke-env SimpleHeuristicsPlayer on a local PS server.

Detailed per-game logging, JSONL output, and win rate graphs (like play_ladder.py).
...
Usage:
    python scripts/play_local_heuristic.py --checkpoint checkpoints/BC-250M/ppo_best.pkl --games 50
    python scripts/play_local_heuristic.py --checkpoint checkpoints/ppo_latest.pkl --games 100 --verbose
    python scripts/play_local_heuristic.py --checkpoint checkpoints/ppo_latest.pkl --vs random --games 20
"""
...
async def run_games(args, log, out_dir):
    ...
    log.info("Playing %d games: PokejaxBot vs %s", args.games, opp_name)
    log.info("Format: %s | Server: localhost:8000", args.format)

    results_file = out_dir / "local_results.jsonl"
    history = []
    ...
    # Play in batches to get per-game results
    while total < args.games:
        batch = min(args.batch_size, args.games - total)
        try:
        ...
        if total % 10 == 0 and history:
            _plot_winrate(history, opp_name, out_dir)
    ...
    with open(out_dir / "local_summary.json", "w") as f:
...
def _plot_winrate(history, opp_name, out_dir):
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "local_winrate.png", dpi=150)
    plt.close()


async def main():
    parser = argparse.ArgumentParser(
        description="Play pokejax vs SimpleHeuristicsPlayer on local PS server"
    )
    parser.add_argument(
    ...
    sh.setFormatter(fmt)
    root.addHandler(sh)
    fh = logging.FileHandler(out_dir / "local_eval.log")
    fh.setFormatter(fmt)
    root.addHandler(fh)
